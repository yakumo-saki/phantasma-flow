...
//   ジョブ終了→ログとかなると困るので終了はタイムアウトのみ （shutdown時は別）
func (m *logFileExporter) Start(params *logListenerParams, wg *sync.WaitGroup) {
	...
	f, err := m.openLogFile(params.JobNumber, params.JobId, params.RunId)
	if err != nil {
...
// openLogFile opens logfile or create logfile and open it.
func (m *logFileExporter) openLogFile(jobNumber int, jobId, runId string) (*os.File, error) {
	existedFilename := m.findJobLogFile(runId)
	if existedFilename == "" {
		return m.createLogFile(jobNumber, jobId, runId)
	} else {
		// open existant log
...
func (m *logFileExporter) createLogFile(jobNumber int, jobId, runId string) (*os.File, error) {
	...
	filename := fmt.Sprintf("%d_%s_%s_%s.json", jobNumber, datetimeStr, runId, jobId)
...
func (m *logFileExporter) findJobLogFile(runId string) string {
	...
	pattern := path.Join(logDir, fmt.Sprintf("*%[1]s*%[1]s%[2]s%[1]s*.json", FILENAME_SEP, runId))
